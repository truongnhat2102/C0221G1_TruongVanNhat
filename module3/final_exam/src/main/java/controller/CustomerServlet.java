...
import model.service.impl_customer.CustomerImpl;
...
//            String address = request.getParameter("address");
            Customer customer = new Customer(1, name, typeCustomer , dateOfbirth, gender);
...
//            String email = request.getParameter("email");
//            String address = request.getParameter("address");
            Customer customer = new Customer(id, name, typeCustomer , dateOfbirth, gender);
            String[] msgArr = iCustomer.edit(customer);
            if (msgArr.length == 0){
                showFormEdit(request,response);
            } else {
                request.setAttribute("msgName", msgArr[0]);
...
//                request.setAttribute("msgPhone", msgArr[4]);
        ...
        private void delete(HttpServletRequest request, HttpServletResponse response) {
            ...
            request.setAttribute("customers",iCustomer.findByAll());
            try {
                requestDispatcher.forward(request, response);
            } catch (ServletException e) {
                e.printStackTrace();
